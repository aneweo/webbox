...
if (isset($_SESSION['user'])) {
    ...
    if (!file_exists($startdir)) {
        mkdir($startdir, 0777);
        mkdir("$startdir/public", 0777);
        mkdir("$startdir/private", 0777);
    }
}
if (request_okay($_GET, 'public-box')) {
    $startdir = 'boxes/' . rtrim($_GET['public-box'], '/') . '/public/';
}
if (request_okay($_GET, 'key')) {
    $owner = $userTools->getOwnerFromKey($_GET['key']);
    $startdir = "boxes/$owner/private/";
}
/*
...
// If the person is only here to view the public box, don't show private
if (isset($_GET['public-box'])) {
    $hide[] = 'private';
}
...
if(!file_exists($opendir)) {
    $_SESSION['alert'] = '<div class="alert fade in alert-error">
            <button class="close" data-dismiss="alert" type="button">&times;</button>
            <strong>Error: </strong>Sorry, this box is unknown to me.
           </div>';
}
